...
typedef struct tag_MMAP_Node
{
    ...
    unsigned long refcount;  /* map后的空间段的引用计数 */
...
int memunmap(void * addr_mapped)
{
    ...
    if(pTMMAPNode == NULL)
    {
        printf("[%s %d] memunmap(): address(%p) have not been mmaped!\n", __func__, __LINE__, addr_mapped);
        return -1;
    }
    ...
    do
	{
		//printf("addr map %x start v %x len %x\n",(unsigned int)addr_mapped,  pTmp->Start_V, (pTmp->Start_V + pTmp->length));
			...
			((unsigned long)addr_mapped <= (pTmp->Start_V + pTmp->length)) )
		{
            ...
            if(0 == pTmp->refcount)
            {
                ...
                {
                    printf("memunmap(): munmap failed err %d(%s)!\n", errno, strerror(errno));
                }
